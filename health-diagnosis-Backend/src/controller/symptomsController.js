  ...
  async symptomsCheck(req, res) {
    ...
    if (matchedSymptomIds.length > 0) {
      const [matchedRows] = await sequelize.query(
        ...
        { replacements: { ids: matchedSymptomIds } }
      );
      ...
      const uniqueDiseaseIds = [...new Set(diseaseIds)];

      const [totalRows] = await sequelize.query(
        `SELECT disease_id, SUM(weight) AS total_weight
         FROM disease_symptoms
         WHERE disease_id IN (:ids)
         GROUP BY disease_id`,
        { replacements: { ids: uniqueDiseaseIds } }
      );
      ...
      const diseases = await Disease.findAll({
        where: { id: { [Op.in]: uniqueDiseaseIds } },
        attributes: ['id', 'name'],
      });
    ...
    const existingIds = new Set(localResults.map(r => String(r.icdCode || r.id)));
    const combined = [
      ...localResults,
      ...icdResults.filter(r => !existingIds.has(String(r.icdCode || r.id))),
    ];
    ...
    return res.json({
      ...
      console.error(err);
      return res.status(500).json({ message: "Lỗi máy chủ khi chẩn đoán triệu chứng" });
    }
  }
