      ...
      <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
        ...
        {activeTab === "precaution" && (
          <div className="space-y-5">
            <h2 className="text-xl font-extrabold text-slate-800 dark:text-slate-100 mb-6 flex items-center gap-3">
              <span className="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-emerald-500/10 text-emerald-600 dark:text-emerald-400">
            ...
            <div className="grid grid-cols-1 gap-4">
              {detail.precautions ? (
                (Array.isArray(detail.precautions) ? detail.precautions : String(detail.precautions).split("\n")).map((p, idx) => {
                  ...
                  const colors = [
                    { bg: "bg-sky-50 dark:bg-sky-950/30", border: "border-sky-500", text: "text-sky-800 dark:text-sky-200", num: "bg-sky-500 text-white" },
                    { bg: "bg-amber-50 dark:bg-amber-950/30", border: "border-amber-500", text: "text-amber-800 dark:text-amber-200", num: "bg-amber-500 text-white" },
                    { bg: "bg-emerald-50 dark:bg-emerald-950/30", border: "border-emerald-500", text: "text-emerald-800 dark:text-emerald-200", num: "bg-emerald-500 text-white" },
                    { bg: "bg-purple-50 dark:bg-purple-950/30", border: "border-purple-500", text: "text-purple-800 dark:text-purple-200", num: "bg-purple-500 text-white" },
                  ];
                  const c = colors[idx % colors.length];
                  return (
                    <div
                      key={idx}
                      className={`flex items-start gap-4 p-5 border-l-4 rounded-2xl ${c.bg} ${c.border}`}
                    >
                      <span className={`flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold ${c.num}`}>
                        {idx + 1}
                      </span>
                      <p className={`text-sm font-semibold ${c.text} leading-relaxed pt-0.5`}>
                        {cleaned}
                      </p>
                    </div>
                  );
                ...
                <>
                  {detail.Precaution_1 && (
                    <div className="flex items-start gap-4 p-5 border-l-4 rounded-2xl bg-sky-50 dark:bg-sky-950/30 border-sky-500">
                      <span className="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold bg-sky-500 text-white">1</span>
                      <p className="text-sm font-semibold text-sky-800 dark:text-sky-200 leading-relaxed pt-0.5">
                        {detail.Precaution_1}
                      </p>
                    </div>
                  )}
                  {detail.Precaution_2 && (
                    <div className="flex items-start gap-4 p-5 border-l-4 rounded-2xl bg-amber-50 dark:bg-amber-950/30 border-amber-500">
                      <span className="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold bg-amber-500 text-white">2</span>
                      <p className="text-sm font-semibold text-amber-800 dark:text-amber-200 leading-relaxed pt-0.5">
                        {detail.Precaution_2}
                      </p>
                    </div>
                  )}
                  {detail.Precaution_3 && (
                    <div className="flex items-start gap-4 p-5 border-l-4 rounded-2xl bg-emerald-50 dark:bg-emerald-950/30 border-emerald-500">
                      <span className="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold bg-emerald-500 text-white">3</span>
                      <p className="text-sm font-semibold text-emerald-800 dark:text-emerald-200 leading-relaxed pt-0.5">
                        {detail.Precaution_3}
                      </p>
                    </div>
                  )}
                  {detail.Precaution_4 && (
                    <div className="flex items-start gap-4 p-5 border-l-4 rounded-2xl bg-purple-50 dark:bg-purple-950/30 border-purple-500">
                      <span className="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold bg-purple-500 text-white">4</span>
                      <p className="text-sm font-semibold text-purple-800 dark:text-purple-200 leading-relaxed pt-0.5">
                        {detail.Precaution_4}
                      </p>
                    </div>
                  )}
