...
export default function HistoryPage() {
  ...
  function handlePrintAppointment(item: AppointmentItem) {
    ...
    const sttNum = String(item.stt_kham || 1).padStart(2, "0");
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  <div class="header">
    <div>
      ...
      <p style="font-size:12px;font-weight:900;text-transform:uppercase;margin-top:2px">${escapeHtml(item.Hospital?.name || 'BỆNH VIỆN ĐA KHOA')}</p>
      <p style="font-size:8px;margin-top:2px">${escapeHtml(item.Hospital?.address || '')}</p>
    </div>
  ...
  <div class="section">
    ...
    <div class="grid2">
      <p><strong>Họ và tên:</strong> <span style="text-transform:uppercase;font-weight:bold">${escapeHtml(user?.First_name || '')} ${escapeHtml(user?.Last_name || '')}</span></p>
      ...
      <p class="span2"><strong>Địa chỉ thường trú:</strong> ${escapeHtml(user?.address || '--')}</p>
  ...
  <div class="section">
    ...
    <div class="grid2">
      <p><strong>Chuyên khoa:</strong> ${escapeHtml(item.Doctor?.specialty || '--')}</p>
      <p><strong>Bác sĩ phụ trách:</strong> ${escapeHtml(item.Doctor?.name || '--')}</p>
      ...
      ${item.insurance_card_number ? `<p class="span2"><strong>Số thẻ BHYT:</strong> <span style="font-family:monospace;font-weight:bold">${escapeHtml(item.insurance_card_number)}</span></p>` : ''}
      ${diseaseName ? `<p class="span2"><strong>Bệnh lý / Triệu chứng:</strong> ${escapeHtml(diseaseName)}</p>` : ''}
      <p class="span2"><strong>Ghi chú:</strong> ${escapeHtml(cleanNotes)}</p>
    </div>
  </div>
  <div class="section">
  ...
  <div class="sig-grid">
    ...
    <div>
      ...
      <p style="font-size:7px">(Ký, ghi rõ họ tên)</p>
      <div class="sig-space"><span style="font-weight:bold;text-transform:uppercase;font-size:9px">${escapeHtml(user?.First_name || '')} ${escapeHtml(user?.Last_name || '')}</span></div>
    </div>
  </div>
</body></html>`;
  ...
  const setParamsTab = (tabName: string) => {
