...
export async function getDoctorProfile() {
  return getRequest("/api/appointments/doctor/profile");
}
...
export async function updateDoctorProfile(data: {
  ...
  return postRequest("/api/appointments/doctor/profile", data, { method: "PUT" });
}

export async function updateTreatment(id: number, result_notes: string, prescription: string) {
  return postRequest(`/api/appointments/${id}/treatment`, { result_notes, prescription }, { method: "PUT" });
}

export async function searchMedicines(query: string) {
  return getRequest(`/api/medicines?q=${encodeURIComponent(query)}`);
}
