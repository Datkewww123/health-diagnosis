...
import { sendLeaveRequest } from "../api/mail";
...
export default function Predict() {
  ...
  const handleOpenLeaveModal = (disease: DiseaseResult) => {
  ...
  const handleSubmitLeaveRequest = async () => {
    ...
    if (!leaveFormData.recipientName.trim() || !leaveFormData.recipientEmail.trim()) {
      setLeaveError("Vui lòng nhập tên và email người nhận");
      return;
    }
    ...
    try {
      const payload = {
        recipientName: leaveFormData.recipientName,
        recipientEmail: leaveFormData.recipientEmail,
        disease: selectedDisease.name_vi || selectedDisease.name,
        days: leaveFormData.days,
        company: leaveFormData.company,
        position: leaveFormData.position,
      };

      await sendLeaveRequest(payload);
      toast.success("Đơn xin nghỉ phép đã gửi thành công!");
      handleCloseLeaveModal();
    } catch (err: any) {
      console.error("[Predict] Leave request error:", err);
      setLeaveError(err.message || "Có lỗi xảy ra khi gửi đơn");
    } finally {
  ...
  async function handleSearch() {
    ...
    try {
      ...
      // Sắp xếp tất cả theo tỉ lệ tương thích cao nhất
