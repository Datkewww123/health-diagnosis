...
class NewsController {
  async getNews(req, res) {
    try {
      ...
      // Nếu không có bài nào trong DB, thử fetch ngay
      if (news.length === 0) {
        console.log('[News] DB trống, thử fetch ngay...');
        try {
          await fetchAndSaveNews();
          const freshNews = await News.findAll({
            where,
            order: [['published_at', 'DESC']],
            limit: Math.min(parseInt(limit) || 20, 50),
            offset: parseInt(offset) || 0,
          ...
          console.warn('[News] Fetch thất bại:', fetchErr.message);
        }
      }
