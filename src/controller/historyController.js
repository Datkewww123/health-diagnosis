...
class HistoryController{
    ...
    async getUserHistory(req, res){
        try{
            ...
            const histories = await History.find({user: req.user._id, action: 'search'}).sort({createAt: -1}).limit(50);
            return res.json({count: histories.length, data: histories});
        }
        catch(err){
    ...
    async getPredictHistory(req, res){
        try{
            ...
            const histories = (await History.find({user: req.user._id, action:'predict'})).sort({createAt: -1}).limit(50);
            return res.json({count: histories.length, data: histories});
        }
        catch(err){
            return res.status(500).json({message: err.message});
