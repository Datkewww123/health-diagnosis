...
class DiseasesController {
  ...
  // Lấy danh sách tất cả bệnh lý (cho autocomplete)
  async listDiseases(req, res) {
    try {
      const diseases = await Disease.findAll({
        attributes: ['id', 'name'],
      ...
      return res.json({
        count: diseases.length,
        data: diseases.map(d => ({ id: d.id, name: d.name }))
      });
    } catch (err) {
      console.error(err);
      return res.status(500).json({ message: "Lỗi máy chủ khi lấy danh sách bệnh" });
    }
  }
}
